...
// ── Constants ──────────────────────────────────────────
const CATS: (keyof ProjectMap)[] = ["webflow", "wordpress", "react", "government"]
...
const EMPTY_PROJECT: Project = { num: "", title: "", domain: "", desc: "", tags: [], url: "", badge: "" }

const DEFAULT_PROJECTS: ProjectMap = {
  webflow: [
    ...
    { num: "04", title: "GoPortal", domain: "goportal.webflow.io", desc: "Portal product marketing website built on Webflow. Clean, conversion-focused layout.", tags: ["SaaS Product", "Portal", "Marketing"], url: "https://goportal.webflow.io" },
    ...
    { num: "08", title: "MVK Education", domain: "mvk.edu.in", desc: "Education institution website. Course listings, admissions flows, faculty showcases.", tags: ["Education", "India", "Admissions"], url: "https://mvk.edu.in" },
    { num: "09", title: "Sparx · Odoscope", domain: "sparx.odoscope.com", desc: "Subdomain product site for Sparx, part of the Odoscope ecosystem.", tags: ["Analytics", "Data Product", "SaaS"], url: "https://sparx.odoscope.com" },
    ...
    { num: "11", title: "FitSpot", domain: "fitspot-wip.webflow.io", desc: "Fitness and wellness platform website with energetic visual language and membership flows.", tags: ["Health & Fitness", "Memberships", "Mobile-First"], url: "https://fitspot-wip.webflow.io" },
    ...
    { num: "18", title: "Fidelco", domain: "fidelco.webflow.io", desc: "Smart home and commercial integration services.", tags: ["Smart Home", "Commercial AV", "Home Theater"], url: "https://fidelco.webflow.io" },
  ],
  wordpress: [
    { num: "WP·01", title: "Yard Athletics", domain: "yardathletics.ca", desc: "Boutique strength & conditioning gym in Canada. 8 service types, MindBody booking integration.", tags: ["Fitness", "E-Commerce", "Booking", "Canada"], url: "https://yardathletics.ca" },
    { num: "WP·02", title: "Edarat Group", domain: "edaratgroup.com", desc: "Enterprise cloud & data center consultancy for MENA/GCC. Full WordPress/Elementor build.", tags: ["Enterprise", "Cloud", "MENA", "Elementor"], url: "https://edaratgroup.com" },
    { num: "WP·03", title: "ManyGiggles", domain: "manygiggles.ca", desc: "AI-powered daycare & childcare management SaaS platform for Canada.", tags: ["SaaS / AI", "Childcare", "Canada", "PIPEDA"], url: "https://lightslategray-bat-143962.hostingersite.com" },
    { num: "WP·04", title: "Your Office Partners", domain: "yourofficepartners.com", desc: "UAE business setup and professional services consultancy. 7 service areas.", tags: ["Business Setup", "UAE / Dubai", "Legal / Finance"], url: "https://yourofficepartners.com" },
  ...
  react: [
    ...
    { num: "RX·03", badge: "React", title: "Parccom", domain: "Parccom Platform", desc: "Enterprise web application built with React, Tailwind CSS, and Shadcn UI.", tags: ["Enterprise App", "React", "Tailwind"], url: "" },
    ...
    { num: "RX·05", badge: "React · TypeScript", title: "Edarat DMS (Frontend Demo)", domain: "dms-demo-frontend-test.vercel.app", desc: "Enterprise document management system demo with role-based permissions.", tags: ["DMS", "Role-Based Access", "TypeScript", "Vite"], url: "https://dms-demo-frontend-test.vercel.app" },
  ],
  government: [
    { num: "GOV·01", badge: "Liferay DXP", title: "SEDD — Sharjah Economic Development Dept", domain: "sedd.ae", desc: "Government digital portal for the Sharjah Economic Development Department, UAE. 230+ services.", tags: ["Government", "UAE / Sharjah", "Liferay DXP", "Bilingual RTL", "230+ Services"], url: "https://sedd.ae/en/web/sedd/home" },
    { num: "GOV·02", badge: "Android / UI", title: "MOHRE Kiosk — Ministry of HR & Emiratisation", domain: "UAE Dubai Government · Dubai", desc: "Complete UI design for the MOHRE Android kiosk support app — a UAE Dubai government project.", tags: ["Government", "Android / Kiosk", "UAE Dubai", "UX Design"], url: "" },
  ],
}
...
const DEFAULT_SITE_TEXT: SiteText = {
  ...
  aboutText1: "I'm Zahid — a UI/UX Designer and Front-End Developer based in Lahore, Pakistan, with deep roots across the UAE, UK, and Canada.",
  aboutText2: "My work spans Webflow, WordPress, React, Next.js, Figma, and Tailwind — and I use AI tools like Claude and Cursor to deliver faster, better.",
...
const LS_SITETEXT = "cms_sitetext"

function loadFromStorage(): { projects: ProjectMap | null; siteText: SiteText | null } {
  try {
    const p = localStorage.getItem(LS_PROJECTS)
    const s = localStorage.getItem(LS_SITETEXT)
    return {
      projects: p ? (JSON.parse(p) as ProjectMap) : null,
      siteText: s ? (JSON.parse(s) as SiteText) : null,
    }
  } catch {
    return { projects: null, siteText: null }
  }
}

function saveToStorage(projects: ProjectMap, siteText: SiteText) {
  try {
    localStorage.setItem(LS_PROJECTS, JSON.stringify(projects))
    localStorage.setItem(LS_SITETEXT, JSON.stringify(siteText))
  } catch {
...
function Field({ label, value, onChange, type = "text", rows, hint }: FieldProps) {
  const s: React.CSSProperties = { width: "100%", background: "#090d13", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 8, color: "#e8eaf0", fontSize: 13, padding: "8px 12px", outline: "none", fontFamily: "inherit", boxSizing: "border-box" }
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: "block", fontSize: 12, color: "#94a3b8", marginBottom: 5, fontFamily: "monospace" }}>
...
// ── ProjectForm ─────────────────────────────────────────
function ProjectForm({ initial, onSave, onClose, category }: { initial: Project; onSave: (p: Project) => void; onClose: () => void; category: string }) {
  const [p, setP] = useState<Project>({ ...EMPTY_PROJECT, ...initial })
  const set = (k: keyof Project) => (v: string) => setP((prev) => ({ ...prev, [k]: v }))
...
// ── ProjectCard ─────────────────────────────────────────
function ProjectCard({ project, category, onEdit, onDelete }: { project: Project; category: string; onEdit: (p: Project) => void; onDelete: (p: Project) => void }) {
  ...
  return (
    <div style={{ background: "#131c2b", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" } as React.CSSProperties}>
        {project.desc}
      </div>
...
// ── SiteTextEditor ──────────────────────────────────────
function SiteTextEditor({ siteText, onChange }: { siteText: SiteText; onChange: (s: SiteText) => void }) {
  const set = (k: keyof SiteText) => (v: string) => onChange({ ...siteText, [k]: v })

  const setStatField = (i: number, k: keyof Stat) => (v: string) => {
    const stats = [...siteText.stats]
    stats[i] = { ...stats[i], [k]: v }
    onChange({ ...siteText, stats })
  }

  const setHeroTag = (i: number, v: string) => {
    const heroTags = [...siteText.heroTags]
    heroTags[i] = v
    onChange({ ...siteText, heroTags })
  }

  const addHeroTag = () => onChange({ ...siteText, heroTags: [...siteText.heroTags, "New Tag"] })

  const removeHeroTag = (i: number) => {
    onChange({ ...siteText, heroTags: siteText.heroTags.filter((_, j) => j !== i) })
  }

  const secLabel = (label: string) => (
    <div style={{ fontSize: 11, fontFamily: "monospace", color: "#e8c87a", letterSpacing: 2, textTransform: "uppercase", marginBottom: 12, marginTop: 24, paddingBottom: 6, borderBottom: "1px solid rgba(232,200,122,0.15)" }}>
      {label}
    </div>
  )
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <button onClick={addHeroTag} style={{ alignSelf: "flex-start", padding: "5px 14px", background: "rgba(232,200,122,0.08)", color: "#e8c87a", border: "1px solid rgba(232,200,122,0.2)", borderRadius: 6, fontSize: 11, cursor: "pointer", fontFamily: "monospace" }}>
            + Add Tag
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        {siteText.stats.map((s, i) => (
          <div key={i} style={{ background: "#090d13", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: 12 }}>
            <input value={s.val} onChange={(e) => setStatField(i, "val")(e.target.value)} placeholder="Value"
            ...
            <input value={s.label} onChange={(e) => setStatField(i, "label")(e.target.value)} placeholder="Label"
      ...
      {secLabel("About Section")}
      <Field label="About label" value={siteText.aboutLabel} onChange={set("aboutLabel")} />
      ...
      <Field label="Email address" value={siteText.email} onChange={set("email")} />
...
// ── ExportPanel ─────────────────────────────────────────
function ExportPanel({ projects }: { projects: ProjectMap }) {
  const [copied, setCopied] = useState("")

  const gen = (cat: keyof ProjectMap) => {
    const items = projects[cat].map((p) => {
      const lines = (Object.keys(p) as (keyof Project)[])
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: "#94a3b8", marginBottom: 16, lineHeight: 1.6 }}>
        ...
        <code style={{ background: "#131c2b", padding: "2px 6px", borderRadius: 4, color: "#e8c87a", fontSize: 12 }}>src/data/projects.js</code>{" "}
        in your portfolio Vite project.
      </p>
      {CATS.map((cat) => (
        <div key={cat} style={{ marginBottom: 16 }}>
          ...
          <pre style={{ background: "#090d13", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8, padding: 12, fontSize: 10, color: "#64748b", overflow: "auto", maxHeight: 100, margin: 0, lineHeight: 1.5 }}>
            {gen(cat).slice(0, 300)}...
          </pre>
        </div>
      ))}
...
export default function PortfolioCMS() {
  const [projects, setProjects] = useState<ProjectMap>(DEFAULT_PROJECTS)
  const [siteText, setSiteText] = useState<SiteText>(DEFAULT_SITE_TEXT)
  const [activeSection, setActiveSection] = useState("projects")
  const [activeCat, setActiveCat] = useState<keyof ProjectMap>("webflow")
  ...
  useEffect(() => {
    const { projects: savedProjects, siteText: savedSiteText } = loadFromStorage()
    if (savedProjects) setProjects(savedProjects)
    if (savedSiteText) setSiteText(savedSiteText)
    setLoaded(true)
  }, [])
  ...
  const saveAll = (p: ProjectMap, s: SiteText) => {
    saveToStorage(p, s)
  }
  ...
  const handleSaveSiteText = () => {
    saveAll(projects, siteText)
  ...
  if (!loaded) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", color: "#94a3b8", fontFamily: "monospace", fontSize: 13, background: "#090d13" }}>
        Loading CMS…
      </div>
    )
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#090d13", color: "#e8eaf0", fontFamily: "'DM Sans',system-ui,sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      {/* ── Header ── */}
      <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(15,21,33,0.98)", position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(232,200,122,0.12)", border: "1px solid rgba(232,200,122,0.25)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>⬡</div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 500 }}>Portfolio CMS</div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {nav.map((n) => (
            <button key={n.key} onClick={() => setActiveSection(n.key)}
              style={{ padding: "6px 14px", background: activeSection === n.key ? "rgba(232,200,122,0.12)" : "transparent", color: activeSection === n.key ? "#e8c87a" : "#94a3b8", border: `1px solid ${activeSection === n.key ? "rgba(232,200,122,0.25)" : "rgba(255,255,255,0.08)"}`, borderRadius: 8, fontSize: 12, cursor: "pointer", fontFamily: "monospace" }}>
              {n.icon} {n.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, padding: 20, maxWidth: 1200, width: "100%", boxSizing: "border-box", margin: "0 auto" }}>
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
              ...
              <button onClick={() => setModal({ mode: "add", project: { ...EMPTY_PROJECT }, index: null })}
                style={{ padding: "8px 18px", background: "rgba(232,200,122,0.12)", color: "#e8c87a", border: "1px solid rgba(232,200,122,0.25)", borderRadius: 8, fontSize: 12, cursor: "pointer", fontFamily: "monospace" }}>
                + Add Project
              </button>
            </div>
        ...
        {activeSection === "sitetext" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 500 }}>Edit site-wide text</h2>
              <button onClick={handleSaveSiteText}
            ...
            <button onClick={handleSaveSiteText}
              style={{ width: "100%", marginTop: 24, padding: "12px 0", background: "rgba(232,200,122,0.12)", color: "#e8c87a", border: "1px solid rgba(232,200,122,0.25)", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: "monospace" }}>
              Save All Site Text
            </button>
          </div>
        )}
